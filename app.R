...
deaths_actual <- c(1,    1,    0,    1,    4,    0,    2,    1,   18,   15,   22,   16,   34,   43,   36,   56,   35,   74,  149,  186,  183,  284,  294,  214,  374,  382,  670,  652,  714,  760,  644,  568, 1038, 1034, 1103, 1152,  839,  686,  744, 1044,  842, 1029,  935, 1115,  498,  559, 1172,  837,  727, 1005,  843,  420,  338,  909,  765, 674, 739, 621, 315, 288, 693, 649, 539, 626, 346, 269, 210)
...
fcast_cases <- function(prop.low.bound = 0.01) {
	...
	prop_fcast_lc <- clipped(prop_fit_lc[n], prop.low.bound, 1)

	prop_fit_fcast_tv <- forecast(prop_actual, 1)
	prop_fit_tv <- as.numeric(prop_fit_fcast_tv$fitted)
	prop_fcast_tv <- clipped(as.numeric(prop_fit_fcast_tv$mean), prop.low.bound, 1)

	prop_fit_pl <- robust.not(prop_actual)
	prop_fcast_pl <- clipped(2 * prop_fit_pl[n] - prop_fit_pl[n-1], prop.low.bound, 1)
