...
#	also uses: https://github.com/pfryz/wild-binary-segmentation-2.0/blob/master/WBS2_SDLL_github_v3.R
	...
	tested_fcast_lc <- clipped(tested_fit_lc[n-9], 0, Inf)
	...
	tested_fcast_pl <- clipped(round(2 * tested_fit_pl[n-9] - tested_fit_pl[n-10]), 0, Inf)


	prop_actual <- (cases_int / tested_int)[7:(n-3)]

	prop_fit_lc <- wbs.sdll.cpt(prop_actual)$est
	prop_fcast_lc <- clipped(prop_fit_lc[n-9], 0, 1)
	...
	prop_fcast_pl <- clipped(2 * prop_fit_pl[n-9] - prop_fit_pl[n-10], 0, 1)
...
radioButtons("radio", h3("Trend estimates (see bottom of right panel for references)"),
	...
	h3(textOutput("f_cases")),
      ...
      plotOutput(outputId = "ts_plot_1"),
			...
			h6("[data source]", tags$a(href="https://en.wikipedia.org/wiki/2020_coronavirus_pandemic_in_the_United_Kingdom", "https://en.wikipedia.org/wiki/2020_coronavirus_pandemic_in_the_United_Kingdom")),
...
server <- function(input, output) {
	...
	output$f_prop <- renderText({
		...
		paste("Next day's predicted proportion of +ve tests:", round(pred_prop, 2))
